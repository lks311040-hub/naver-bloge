...
PORT=4001 API_PORT=4001 npm run dev   # a 2nd instance beside a running one (e.g. comparing a worktree). Both vars
                               # must match — PORT is Express, API_PORT is what Vite proxies to. Vite picks its own
                               # free port. Each checkout has its own server/data/app.db, so no data is shared.
...
2. **No code enters Naver login credentials.** `naverLogin.ts` opens a real headful Edge window and waits (polling for the `NID_SES` cookie, then resolving the blogId) for a human to log in manually; it never touches the login form fields.
...
4. `server/src/modules/posts/assemble.ts` — a **pure function**, no Playwright/AI imports — wraps the AI-authored body blocks with the fixed template: greeting → talktalk → *body* → prewritten content → reservation link → address link → related-post link → hashtags. Greeting and hashtags always apply; the talktalk/reservation/address trio is per-post-type on/off, read from `profile.attachments[postType]` and edited in the web UI's "글 종류별 첨부 요소" dashboard. Defaults (`DEFAULT_ATTACHMENTS` in `shared/src/business-profile.ts`, mirrored by migration 0006's column DEFAULTs — keep the two in sync) reproduce the pre-dashboard behavior: promotional got talktalk+reservation, informational got neither. The related-post link stays promotional-only and is not part of the dashboard, since it's per-post input rather than a business-profile field. Any template piece whose source field is empty in the business profile is just omitted, never inserted blank — a toggle that's on with an empty URL inserts nothing.
...
- **`progressBus` has no buffer, so a fast-failing run reports nothing at all.** Both automation entry points (`POST /api/naver/login`, `POST /api/posts/:id/autofill`) answer `202 {runId}` immediately and start the work fire-and-forget; the browser only subscribes to `GET /api/events/:runId` *after* that response lands. `progressBus` is a bare `EventEmitter` — an `emit` with no listener yet is dropped, not queued. So any failure that happens in the first few milliseconds (the not-logged-in guard was the real-world one) is published into the void, and the UI sits there looking like the button did nothing. The autofill route now pre-checks the login state and returns `409 {message}` instead, which `apiFetch` surfaces verbatim as the error text. **If you add another early bail-out, return it as an HTTP error too** — don't just `log(..., "error")` and expect anyone to see it. A general fix would be per-`runId` buffering in the bus.
- **Naver's post-login landing page moved, and it silently broke login entirely.** `blog.naver.com/` used to redirect a logged-in user to their own blog; it now lands on the homefeed (`section.blog.naver.com/BlogHome.naver`), which contains no trace of your own blogId — so blogId detection failed, the session was never persisted, and every login attempt ended with the app still showing "로그인 안 됨". `blog.naver.com/MyBlog.naver` still redirects correctly (→ `blog.naver.com/<blogId>`) and is what `tryDetectBlogId` uses now. **Take the blogId from the page URL, never by scraping path-form `blog.naver.com/<id>` links out of HTML** — the homefeed is full of strangers' blog links and you would happily save someone else's blog as the operator's. The `?blogId=` query form is only consulted while actually on `blog.naver.com`.
...
There's no way to know Naver's actual editor DOM, click targets, redirect targets, or preview-card behavior from documentation — it isn't documented. Every automation feature in this repo's history was built by: writing/overwriting `server/scripts/inspect-editor.mjs` to poke at a real logged-in session, reading the actual DOM/screenshots it dumps to `tmp-inspect/` (gitignored), implementing against what was actually observed, then re-running live to confirm. Treat a change to `automation/` or `selectors.ts` as unverified until it's been run against a real session — `npm run verify` passing only proves the types line up, not that Naver's UI behaves as assumed.

This applies to `naverLogin.ts` just as much as to the editor, and that's easy to forget because login has no selectors in it — the URLs Naver redirects to are every bit as undocumented and unstable as its markup (see the `MyBlog.naver` entry above). The same throwaway-probe approach works: launch Edge with the saved `storageState`, visit the candidate URLs, print where you actually land and what the page contains, then write the detection against that.

A stale session makes this harder to diagnose than it looks: the saved cookies expire server-side after a few idle days, and Naver then serves a normal-looking logged-out page rather than an error. When a login-related bug is under investigation, check whether `server/data/naver-storage-state.json` is actually still valid before believing anything else the flow reports.
