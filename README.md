...
로컬 PC에서 실행되는 반자동 네이버 블로그 글 작성/발행 도구입니다.
...
3. **새 글 작성**: 맨 위 **"글 종류"**에서 홍보성/정보성을 먼저 고릅니다 — 고른 값에 따라 아래 입력칸이 바뀝니다. 홍보성은 제목을 직접 입력하고(그대로 사용), 정보성은 제목 칸이 없어지고 주제 키워드만 받아 AI가 제목까지 만듭니다. 이어서 키워드/강조 내용을 입력 → AI 초안 생성 (보통 3~9분 소요, 실시간 로그 표시).
