...
import type { AiBodyBlockList, Block, BusinessProfileRecord } from "@app/shared";
...
/**
 * Pure function — the fixed 7-step template assembly. Never touches
 * Playwright or the AI SDK, so it's reusable from both the manual generate
 * path and the (future) scheduler path.
 *
 * Order: 인사말 → 톡톡 → AI 본문 → 직접쓴글(줄바꿈=문단) → 예약 link_block →
 * 관련글 link_block → 해시태그. Any step whose source field is empty is
 * simply omitted (e.g. no reservation URL registered yet -> no reservation
 * block), rather than inserting an empty placeholder.
 */
